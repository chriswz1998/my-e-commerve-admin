...
import { Nav2, Nav3 } from '@prisma/client'
...
type NavTFormValues = z.infer<typeof formSchema>

export const NavTForm = ({ initialData, nav2 }: NavTFormProps) => {
  ...
  const form = useForm<NavTFormValues>({
  ...
  const onSubmit = async (data: NavTFormValues) => {
    try {
      setLoading(true)
      if (initialData) {
  ...
  const onDelete = async () => {
    try {
      setLoading(true)
      await axios.delete(`/api/nav3/${params.nav3Id}`)
      route.push(`/nav3s`)
      route.refresh()
